    ...
    internal static class PropertyCopier<TSource, TTarget>
    {
        ...
        internal static void Copy(TSource source, TTarget target)
        {
            ...
            for (int i = 0; i < SourceProperties.Count; i++)
            {
                ...
                // foreach (var prop in SourceProperties[i].GetValue(source, null).GetType().GetProperties())
                // {
                ...
                //
                // for (var j = 0;
                //         j < SourceProperties[i].GetValue(source, null).GetType().GetProperties(BindingFlags.Instance).Length;
                //         j++)
                //     {
                //
                //         if(SourceProperties[i].GetValue(source, null).GetType().GetProperties()[j].Name == "Id")
                //             continue;
                //         
                //         if (SourceProperties[i].GetValue(source, null).GetType().GetProperties()[j].Name != "Id")
                //         {
                //             TargetProperties[i].GetType().GetProperties()[j].SetValue(target,
                //                 SourceProperties[i].GetValue(source, null).GetType().GetProperties()[j].GetValue(source, null), null);
                //         }
                //     }
                // var indexChildProp = 0;
                    foreach (var childProperty in SourceProperties.GetType().GetProperties())
                    {
                        //Copy(childProperty, TargetProperties[i].GetType().GetProperties()[indexChildProp]).GetType().GetProperties()[];
                        // TargetProperties[i].GetType().GetProperties()[indexChildProp].SetValue(target.);
                        // indexChildProp++;
                    }    
